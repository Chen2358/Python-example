...
html_folder = '/media/backup/html/acfun/'
...
def del_video():
    ...
    for video in video_collection.find({'$and':[{'time':{'$lt':start_time}}, {'video_download_status':2}]}):
        try:
            print('del ', os.path.join(html_folder, video['video_filename']))
            os.remove(os.path.join(html_folder, video['video_filename']))
...
def find_video(folder, folderInMongo, video):
    video_filename = None
...
def find_json(folder, folderInMongo, video):
    json_filename = None
    for filename in os.listdir(str(folder)):
    ...
    if json_filename != None:
        video_collection.find_and_modify({"_id" : video['_id']}, \
                                         update={'$set':{'json_filename':json_filename}}, \
                                         upsert=True, new=True)
        return True
    return False
...
def findAndDownload():
    ...
    while 1:
        try:
            ...
            find_video(folder, folderInMongo, video)
            find_json(folder, folderInMongo, video)
            ...
            if video:
                videoTag = find_video(folder, folderInMongo, video)
                jsonTag = find_video(folder, folderInMongo, video)
                if not videoTag:
                    retries += 1
                    video_collection.update({"_id" : video['_id']}, \
                                            {'$set':{'video_download_status':0}})
