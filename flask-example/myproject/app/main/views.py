...
from . import main
from .forms import EditProfileForm, EditProfileAdminForm, PostForm, CommentForm, AddUserAdminForm
from .. import db
...
def after_request(response):
	for queryr in get_debug_queries():
...
def userlist():
	page = request.args.get('page', 1, type=int)
	pagination = User.query.order_by(User.username.desc()).paginate(
		page, per_page=current_app.config['FLASKY_USERS_PER_PAGE'],
		error_out=False)
	users = pagination.items
	return render_template('userlist.html', users=users, pagination=pagination, page=page, title="User List")
