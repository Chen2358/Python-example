...
class Nude(object):
    ...
    Skin = namedtuple("Skin", "id skin region x y")				#collection.namedtuple()函数是一个返回python标准元组类型的工厂方法，需要传递一个类型名和需要的字段，它会返回一个类，可以初始化这个类，
    ...
    def parse(self):
        ...
        for y in range(self.height):
            for x in range(self.width):
                ...
                # *^
                """
				相邻像素的索引值，因为像素的 id 值是从 1** 开始编起的，而索引是从 **0 编起的。变量 _id 是存有当前像素的 id 值， 
				所以当前像素在 self.skin_map 中的索引值为 _id - 1，
				以此类推，那么其左方的相邻像素在 self.skin_map 中的索引值为 _id - 1 - 1 ，
				左上方为 _id - 1 - self.width - 1，上方为 _id - 1 - self.width ，右上方为 _id - 1 - self.width + 1
                """	
                ...
                if region == -1:
                    ...
                    _skin = self.skin_map[_id - 1]._replace(region=len(self.detected_regions))		#somenamedtuple._replace(kwargs)返回一个替换指定字段的值为参数的namedtuple实例
    ...
    def _add_merge(self, _from, _to):
        ...
        # 索引位置和对应值可使用enumerate()得到，即index，region
    ...
    # 皮肤区域清理函数
    # 只保存像素数大于指定数量的皮肤区域，保存到self.skin_regions
...
if __name__ == "__main__":
	...
	(1).argparse 模块使得编写用户友好的命令行接口非常容易。
	(2).程序只需定义好它要求的参数，然后 argparse 将负责如何从 sys.argv 中解析出这些参数。
	(3).argparse 模块还会自动生成帮助和使用信息并且当用户赋给程序非法的参数时产生错误信息
